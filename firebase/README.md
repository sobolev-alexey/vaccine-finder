...
`load.get('/load?key=aa7e46cae0ayfq7b18')`
...
```
gsutil cors get gs://cityexchange-locationtracking.appspot.com
```

Use the `gsutil cors` command to configure CORS on a bucket:

```
gsutil cors set cors-config.json gs://cityexchange-locationtracking.appspot.com
```
